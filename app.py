...
<head>
  <meta charset="utf-8">
  <title>Lightweight Coin Counter - Webcam</title>
  <style>
...
<body>
  <h2>Lightweight Coin Counter (Web) - Webcam Capture</h2>
  <div>
...
document.getElementById('openCamera').addEventListener('click', async ()=>{
  if(stream){
  stream.getTracks().forEach(t=>t.stop()); stream=null; video.srcObject=null; return;
  }
  try{
...
document.getElementById('captureBtn').addEventListener('click', async ()=>{
  ...
  img.src = url; img.onload = ()=>{
    img_w = off.width; img_h = off.height; fitCanvasToImage(img_w, img_h);
    markers = [];
    draw();
  };
  if(document.getElementById('autoDetect').checked){
    ...
    if(data.error){ alert(data.error); return; }
    img.src = data.annotated; img.onload = ()=>{
    img_w = data.width; img_h = data.height; fitCanvasToImage(img_w, img_h);
    markers = data.markers.map(m=>({x:m[0], y:m[1], r:m[2]}));
    updateLogTable(); draw();
    };
  }
